...
contract DeWildClub is ERC721, ERC2981, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  function mint() external payable nonReentrant {
    ...
    function _update(
        ...
        // Если это не минт (from != address(0)) и не сжигание (to != address(0))
        if (from != address(0) && to != address(0)) {
            // Если токен еще не был продан и отправитель не primaryMarket
            if (!hasBeenSold[tokenId] && from != primaryMarket) {
                // Проверяем, установлен ли primaryMarket
                require(primaryMarket != address(0), "Primary market not set");
                // Разрешаем трансфер только на primaryMarket
